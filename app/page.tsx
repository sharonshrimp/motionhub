...
export default function MotionHub() {
  ...
  // --- 關鍵修復：確保 currentWeek 在 JSX 使用前定義 ---
  ...
  if (loading) return <div className="min-h-screen bg-white flex items-center justify-center text-slate-400 font-bold tracking-tighter italic">YC MOTION SYNCING...</div>;
    ...
    <main className="min-h-screen bg-[#FDFDFD] text-slate-800 p-5 pb-24 font-sans">
      <div className="max-w-md mx-auto">
        ...
        <header className="mb-6 flex justify-between items-center px-1">
          ...
          <button onClick={() => { setViewDate(new Date()); setSelectedDate(new Date().toLocaleDateString('en-CA')); }} className="text-[10px] font-black bg-slate-100 px-4 py-2 rounded-full text-slate-500 hover:text-indigo-600 transition-all">回今天</button>
        </header>
        ...
        <section className="bg-white rounded-[2.5rem] p-6 mb-6 shadow-sm border border-slate-100 border-t-4 border-t-indigo-500">
          <div className="flex justify-between items-center mb-5">
            <h3 className="text-[11px] font-black text-slate-800 tracking-widest uppercase">{editingId ? '編輯' : '新增'} {category}</h3>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {categories.map(cat => (
              <button key={cat} onClick={() => { setCategory(cat); resetForm(); }} className={`px-3 py-1.5 rounded-lg text-[10px] font-black transition-all ${category === cat ? 'bg-indigo-500 text-white shadow-md' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>{cat}</button>
            ))}
          </div>
          ...
          <div className="space-y-3">
            ...
            {category === '重訓' && (
              <div className="grid grid-cols-3 gap-2">
                ...
                <input type="number" placeholder="次數" className="bg-slate-50 p-4 rounded-xl text-center font-bold outline-none" value={reps} onChange={(e) => setReps(e.target.value)} />
                <input type="number" placeholder="組數" className="bg-slate-50 p-4 rounded-xl text-center font-bold outline-none ring-1 ring-indigo-50" value={setsCount} onChange={(e) => setSetsCount(e.target.value)} />
              </div>
            )}
            ...
            {(category === '網球' || category === '其他' || category === '伸展') && (
              <textarea className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none border border-transparent focus:border-indigo-100 min-h-[100px]" placeholder={category === '網球' ? '記錄練習對象、內容...' : (category === '伸展' ? '內容描述' : '內容細節...')} value={note} onChange={(e) => setNote(e.target.value)} />
            )}
          </div>
        ...
        <section className="space-y-3">
          <div className="flex justify-between px-3 items-center mb-2">
            ...
            <span className="text-[12px] font-black text-indigo-600">{dayData.totalVolume?.toLocaleString()} KG</span>
          </div>
          
          {dayData.exercises?.map((ex: any) => (
            <div key={ex.id} onClick={() => startEdit(ex)} className="bg-white px-5 py-5 rounded-[2rem] border border-slate-100 shadow-sm flex justify-between items-center cursor-pointer hover:shadow-md transition-all animate-in fade-in slide-in-from-bottom-2">
